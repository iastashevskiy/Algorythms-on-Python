# 1. Написать программу, которая будет складывать, вычитать, умножать или делить два 
# числа. Числа и знак операции вводятся пользователем. После выполнения вычисления 
# программа не должна завершаться, а должна запрашивать новые данные для вычислений. 
# Завершение программы должно выполняться при вводе символа '0' в качестве знака 
# операции. Если пользователь вводит неверный знак (не '0', '+', '-', '*', '/'), 
# то программа должна сообщать ему об ошибке и снова запрашивать знак операции. 
# Также сообщать пользователю о невозможности деления на ноль, если он ввел 0 в 
# качестве делителя.


a = float(input('Введите первое число: '))
b = float(input('Введите второе число: '))
op = input('Введите знак операции. Для выхода введите 0. ')


# Я извиняюсь за то, что произойдет ниже. Оно мне не нравится и я нне могу с ходу 
# сделать это красивее. Но хотя бы работает. Вроде


while op != '0':
	while op != '+' and op != '-' and op != "*" and op != "/":
		op = input('Введен некорректный знак операции. Введите еще раз: ')
	while op == '/' and b == 0:
		b = float(input('Деление на ноль невозможно. Введите второе число: '))
	if op == '+':
		c = a + b
		print('Результат вычислений = ', c)
		a = float(input('Введите первое число: '))
		b = float(input('Введите второе число: '))
		op = input('Введите знак операции. Для выхода введите 0. ')
	elif op == '-':
		c = a - b
		print('Результат вычислений = ', c)
		a = float(input('Введите первое число: '))
		b = float(input('Введите второе число: '))
		op = input('Введите знак операции. Для выхода введите 0. ')
	elif op == '*':
		c = a * b
		print('Результат вычислений = ', c)
		a = float(input('Введите первое число: '))
		b = float(input('Введите второе число: '))
		op = input('Введите знак операции. Для выхода введите 0. ')
	elif op == '/':
		c = a / b
		print('Результат вычислений = ', c)
		a = float(input('Введите первое число: '))
		b = float(input('Введите второе число: '))
		op = input('Введите знак операции. Для выхода введите 0. ')

else:
	print('Bye')
	quit()

